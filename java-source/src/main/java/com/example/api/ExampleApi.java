...
public class ExampleApi {
    ...
    //update the  exisiting state value in corda
    @PUT
    @Path("consumed")
   // @Produces(MediaType.APPLICATION_JSON)
    public Response getMYIOUConsumed() throws NoSuchFieldException {
        CordaX500Name partyName = new CordaX500Name("PartyB", "New York", "US");
int iouValue=35;
...
//        List<StateAndRef<IOUState>> results = rpcOps.vaultQueryByCriteria(criteria,IOUState.class).getStates();
      ...
      //  List<IOUState> st = (List<IOUState>) results.stream().map(v ->v.getState());
        final Party otherParty = rpcOps.wellKnownPartyFromX500Name(partyName);

            try {
                final SignedTransaction signedTx = rpcOps
                        .startTrackedFlowDynamic(ExampleFlow.UpdateFlow.class, 35, otherParty)
                        .getReturnValue()
                        .get();
    ...
    public Response createIOU(@QueryParam("iouValue") int iouValue, @QueryParam("partyName") CordaX500Name partyName) throws InterruptedException, ExecutionException {
       logger.info("CordaX500Name==============="+ partyName);
        logger.info("iouValue==============="+ iouValue);
    ...
    @Path("unconsumed")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getMYIOUUnConsumed() throws NoSuchFieldException {
